...
void setup() {
    ...
    attachInterrupt(digitalPinToInterrupt(InterruptPin), onAlarm, CHANGE);
    Serial.println("wtf");
    ...
    if(!rtc.setAlarm1(
      DateTime(2022, 8, 2, 3, 12, 10), DS3231_A1_Hour
    )) {
    ...
    if(!rtc.setAlarm2(
      DateTime(2022, 8, 2, 3, 12, 50), DS3231_A2_Hour
    )) {
      Serial.println("Error, alarm wasn't set!");
    }else {
...
void loop() {
    ...
    if(rtc.alarmFired(1)) {
        rtc.clearAlarm(1);
        rtc.setAlarm1(rtc.now() + TimeSpan(0, 0, 0, 15), DS3231_A1_Hour);
        Serial.println("Alarm 1 cleared");
    }

    if(rtc.alarmFired(2)) {
        rtc.clearAlarm(2);
        rtc.setAlarm2(rtc.now() + TimeSpan(0, 0, 0, 15), DS3231_A2_Hour);
